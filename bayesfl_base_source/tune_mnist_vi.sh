#!/usr/bin/env bash
# Hyperparameter sweep for VI Bayesian FL on MNIST non-IID unbalanced.
# Search variables: vi_lr, batch_size, local_epochs.
# Run with:
#   nohup bash tune_mnist_vi.sh > logs/tune_mnist_vi.nohup.log 2>&1 &
...
# VI constants. We tune VI_LRS, batch size, and local epochs below.
...
ROOT_OUT="${ROOT_OUT:-outputs/tune_vi_mnist_noniid_unbalanced}"
ROOT_PLOTS="${ROOT_PLOTS:-plots/tune_vi_mnist_noniid_unbalanced}"
...
# Overrides:
#   FORCE_DEVICE=cpu
#   FORCE_DEVICE=cuda CUDA_VISIBLE_DEVICES=0 CLIENT_GPUS=0.5
FORCE_DEVICE="${FORCE_DEVICE:-auto}"
CLIENT_GPUS="${CLIENT_GPUS:-0.5}"  # VI is heavier than FedAvg/OLA.
...
# Hyperparameter grid
# ------------------------------------------------------------
# Default quick grid = 8 runs.
# Full grid example:
#   GRID_MODE=full NUM_ROUNDS=60 nohup bash tune_mnist_vi.sh > logs/tune_mnist_vi.log 2>&1 &
GRID_MODE="${GRID_MODE:-quick}"

if [[ "${GRID_MODE}" == "full" ]]; then
  VI_LRS=(0.002 0.001 0.0005 0.0002)
...
cat > "${SWEEP_CSV}" <<'CSV'
run_label,status,method,vi_lr,batch_size,local_epochs,client_fraction,num_rounds,final_round,final_global_accuracy,final_global_loss,final_global_ece,final_global_nll,final_local_accuracy_weighted,final_posterior_sigma_mean,final_posterior_snr_raw_p50,output_dir
CSV
...
append_result() {
  ...
  RUN_LABEL="${run_label}" RUN_STATUS="${status}" VI_LR="${vi_lr}" BATCH_SIZE="${batch_size}" LOCAL_EPOCHS_RUN="${local_epochs}" CLIENT_FRACTION_RUN="${CLIENT_FRACTION}" NUM_ROUNDS_RUN="${NUM_ROUNDS}" OUT_DIR="${out_dir}" SWEEP_CSV="${SWEEP_CSV}" python - <<'PY'
import csv, os, math
...
run_one() {
  ...
  local lr_tag
  lr_tag="$(safe_tag "${vi_lr}")"
  local label="vi_lr${lr_tag}_b${batch_size}_e${local_epochs}"
  ...
  if [[ ${status} -eq 0 ]]; then
    echo "[ok] ${label}"
    append_result "${label}" "ok" "${vi_lr}" "${batch_size}" "${local_epochs}" "${out_dir}"
  else
    echo "[fail] ${label}; see ${log_file}"
    append_result "${label}" "fail" "${vi_lr}" "${batch_size}" "${local_epochs}" "${out_dir}"
  fi
}
...
for vi_lr in "${VI_LRS[@]}"; do
  for batch_size in "${BATCH_SIZES[@]}"; do
    for local_epochs in "${LOCAL_EPOCHS[@]}"; do
      run_one "${vi_lr}" "${batch_size}" "${local_epochs}"
    done
  done
done
stop_ray
...
echo
echo "============================================================"
echo "VI sweep finished. elapsed_seconds=${SECONDS}"
