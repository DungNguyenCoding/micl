...
# Sparse-selection ablation for VI Bayesian FL.
# Compares Bayesian update-SNR top-k selection against random top-k selection
# under identical keep ratios and training settings.
...
for SELECTION in "${SELECTIONS[@]}"; do
  for KEEP_RATIO in "${KEEP_RATIOS[@]}"; do
    ...
    echo "vi,${SELECTION},${KEEP_RATIO},${RUN_DIR},${LOG_FILE}" >> "${MANIFEST}"
    ...
    if [[ "${VI_USE_DECAY}" == "true" ]]; then
      DECAY_ARGS=(--vi_lr_decay_milestones 80,120,160 --vi_lr_decay_gamma 0.5 --vi_max_scale 0.5)
    fi

    echo "===== Running VI ${SELECTION} keep=${KEEP_RATIO} ====="
    echo "Output: ${RUN_DIR}"
    echo "Log:    ${LOG_FILE}"
...
echo "===== VI sparse-selection ablation finished ====="
