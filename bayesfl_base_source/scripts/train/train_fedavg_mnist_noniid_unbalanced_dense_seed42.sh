...
# FedAvg final comparison run: MNIST / non-IID / unbalanced
# Same experimental environment as VI/OLA comparison, but with FedAvg hyperparameters.
# Run:
#   nohup bash fa_mnist_noniid_unbalanced.sh > logs/fa_mnist_noniid_unbalanced.log 2>&1 &
# Optional multi-seed:
#   SEEDS="42 43 44" nohup bash fa_mnist_noniid_unbalanced.sh > logs/fa_mnist_noniid_unbalanced.log 2>&1 &
...
for SEED in ${SEEDS}; do
  OUT_DIR="${BASE_OUT}/fedavg_seed${SEED}"
  echo
  echo "============================================================"
  echo "FedAvg final comparison | seed=${SEED} | rounds=${NUM_ROUNDS}"
  echo "Output: ${OUT_DIR}"
  echo "============================================================"
...
echo
echo "FedAvg final comparison finished."
echo "Outputs: ${BASE_OUT}/fedavg_seed*"
