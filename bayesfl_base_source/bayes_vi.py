...
def _functional_call_module(net: torch.nn.Module, params: OrderedDict[str, torch.Tensor], x: torch.Tensor) -> torch.Tensor:
    """Run ``net`` with an explicit parameter dictionary.

    ``torch.func.functional_call`` is preferred on recent PyTorch versions.  The
    stateless fallback keeps compatibility with older versions.
    """
    try:
...
def _build_generic_spec(net: torch.nn.Module, num_classes: int) -> GenericTorchModelSpec:
    names: List[str] = []
    shapes: List[Tuple[int, ...]] = []
    ...
    return GenericTorchModelSpec(names=names, shapes=shapes, num_classes=int(num_classes))


def _generic_flat_to_param_dict(flat: np.ndarray | torch.Tensor, spec: GenericTorchModelSpec, device: torch.device) -> OrderedDict[str, torch.Tensor]:
    flat_t = torch.as_tensor(flat, dtype=torch.float32, device=device).flatten()
    if flat_t.numel() != spec.latent_dim:
        raise ValueError(f"Expected latent dim {spec.latent_dim}, got {flat_t.numel()}")
    ...
    for name, shape in zip(spec.names, spec.shapes):
        n = int(np.prod(shape))
        params[name] = flat_t[cursor : cursor + n].view(shape)
        cursor += n
    return params


@torch.no_grad()
def _generic_mean_posterior_nll(trainloader: DataLoader, flat_loc: np.ndarray, net: torch.nn.Module, spec: GenericTorchModelSpec, device: torch.device) -> float:
    net.eval()
    total_loss = 0.0
    total_examples = 0
    params = _generic_flat_to_param_dict(flat_loc, spec, device)
    for x, y in trainloader:
        x = x.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)
        logits = _functional_call_module(net, params, x)
        loss = F.cross_entropy(logits, y, reduction="sum")
        total_loss += float(loss.detach().cpu())
        total_examples += int(y.numel())
    return float(total_loss / max(total_examples, 1))
...
def _make_generic_pyro_model(net: torch.nn.Module, spec: GenericTorchModelSpec, prior_loc: torch.Tensor, prior_scale: torch.Tensor):
    import pyro
    import pyro.distributions as dist

    def bnn_model(x: torch.Tensor, y: torch.Tensor | None = None) -> torch.Tensor:
        flat = pyro.sample("flat_weights", dist.Normal(prior_loc, prior_scale).to_event(1))
        params = _generic_flat_to_param_dict(flat, spec, x.device)
        logits = _functional_call_module(net, params, x)
        with pyro.plate("data", logits.shape[0]):
            pyro.sample("obs", dist.Categorical(logits=logits), obs=y)
        return logits
...
def _train_vi_local_generic(
    ...
    """Run local VI for any model supported by ``model.build_model``.

    This path is used for CNN/ResNet VI. It samples one flat latent parameter
    vector and executes the deterministic Torch model through functional_call.
    """
    import pyro
    ...
    net = model_lib.build_model(cfg, input_shape=input_shape, num_classes=num_classes).to(device)
    net.train()
    spec = _build_generic_spec(net, num_classes=num_classes)
    global_loc_t = torch.as_tensor(global_loc, dtype=torch.float32, device=device).flatten()
    scale_np = np.maximum(np.asarray(global_scale, dtype=np.float32), float(cfg.vi_min_scale))
    global_scale_t = torch.as_tensor(scale_np, dtype=torch.float32, device=device).flatten()
    if global_loc_t.numel() != spec.latent_dim:
        raise ValueError(f"Global VI loc has {global_loc_t.numel()} parameters, but {cfg.model} expects {spec.latent_dim}")
    bnn_model = _make_generic_pyro_model(net, spec, global_loc_t, global_scale_t)
    guide = AutoDiagonalNormal(
        bnn_model,
        init_loc_fn=init_to_value(values={"flat_weights": global_loc_t}),
    ...
    guide(x0, y0)
    try:
        pyro.param("AutoDiagonalNormal.loc").data.copy_(global_loc_t)
        constrained_scale = pyro.param("AutoDiagonalNormal.scale")
        constrained_scale.data.copy_(global_scale_t)
        if hasattr(constrained_scale, "unconstrained"):
            constrained_scale.unconstrained().data.copy_(_softplus_inverse(global_scale_t))
    except Exception:
        pass

    svi = SVI(bnn_model, guide, Adam({"lr": float(cfg.vi_lr)}), Trace_ELBO(num_particles=int(cfg.vi_particles)))
    ...
    vi_likelihood = _generic_mean_posterior_nll(trainloader, loc, net, spec, device)
...
def train_vi_local(
    ...
    """
    if str(getattr(cfg, "model", "mlp")) != "mlp":
        return _train_vi_local_generic(
